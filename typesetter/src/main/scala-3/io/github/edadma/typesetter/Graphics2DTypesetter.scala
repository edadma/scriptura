...
class Graphics2DTypesetter(g: Graphics2D) extends Typesetter:
  ...
  def setColor(color: Color): Unit = g.setColor(color.toJavaColor)

  def drawString(text: String, x: Double, y: Double): Unit = g.drawString(text, x.toFloat, y.toFloat)
  def drawLine(x1: Double, y1: Double, x2: Double): Unit = g.drawLine(x1.toInt, y1.toInt, x2.toInt, y2.toInt)
