...
def registerScripturaPrimitives(proc: Processor, handler: ScripturaHandler): Unit =
  ...
  // font - 3 braced args
  proc.registerPrimitive("font", new Primitive {
    def execute(proc: Processor, pos: CharReader): Unit =
  ...
  // image - 1 arg
  proc.registerPrimitive("image", new Primitive {
    def execute(proc: Processor, pos: CharReader): Unit =
      val arg = evalArg(proc, pos)
      arg match
  ...
  // vskip - 1 braced arg
  proc.registerPrimitive("vskip", new Primitive {
    def execute(proc: Processor, pos: CharReader): Unit =
      val arg = evalArg(proc, pos)
      arg match
  ...
  // hskip - 1 braced arg
  proc.registerPrimitive("hskip", new Primitive {
    def execute(proc: Processor, pos: CharReader): Unit =
      val arg = evalArg(proc, pos)
      arg match
        case Value.Num(d) => t.glue(d.toDouble, 0, 0)
  ...
  proc.registerPrimitive("hrule", new Primitive {
    def execute(proc: Processor, pos: CharReader): Unit =
  ...
  proc.registerPrimitive("hbox", new Primitive {
    def execute(proc: Processor, pos: CharReader): Unit =
      val opts = proc.readOptionalParams(pos)
  ...
  proc.registerPrimitive("vbox", new Primitive {
    def execute(proc: Processor, pos: CharReader): Unit =
      val opts = proc.readOptionalParams(pos)
      val body = proc.readArgument(pos)
      val toVal = opts.get("to").collect { case Value.Num(n) => n.toDouble }.map(java.lang.Double.valueOf).orNull
  ...
  // italic - 1 body arg
  proc.registerPrimitive("italic", new Primitive {
    def execute(proc: Processor, pos: CharReader): Unit =
      val body = proc.readArgument(pos)
      t.italic()
      proc.processTokenList(body)
  ...
  proc.registerPrimitive("smallcaps", new Primitive {
    def execute(proc: Processor, pos: CharReader): Unit =
      val body = proc.readArgument(pos)
      t.smallcaps()
      proc.processTokenList(body)
